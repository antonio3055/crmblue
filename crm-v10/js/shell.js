...
/* Every stored preference and its default. Load and save walk this one list,
   so adding a preference never means editing three places. scaleAuto is
   derived from whether a scale was ever chosen, so it is not stored. */
const PREF_DEFAULTS = {
  ...
  savedFilters: [],
  scanPageLimit: 10,          /* Scanner: approved default scan page count */
  scanOcrDefault: 'auto'      /* Scanner: off | auto | always */
};
const prefs = Object.assign({ scaleAuto: true }, PREF_DEFAULTS);
...
function loadPrefs() {
  try {
    ...
    if (stored.scalePreset !== undefined) prefs.scaleAuto = false;
  } catch (e) { /* private mode or blocked storage — run with defaults */ }
}
function savePrefs() {
  try {
    const out = {};
    Object.keys(PREF_DEFAULTS).forEach(k => { out[k] = prefs[k]; });
...
function applyScaleAndFont() {
  ...
  /* Sized in real pixels rather than vw/vh: inside a transformed element
     viewport units are resolved differently by different engines. visualViewport
     is used when available since it reflects the actual visible area (it does
     not include a mobile browser's address/tab bar), so the app fills exactly
     to the real edges instead of leaving a gap sized to whatever chrome is
     currently showing. */
...
function bootShell() {
  ...
  /* refit() re-applies the desktop-view scale on a real layout-width change
     (window resize, rotation). It must NOT fire while the person is actively
     pinch-zoomed: doing so resets #app's transform mid-gesture and snaps the
     zoom back to the start, which is why a small tightening of the fit used
     to interrupt a pinch that wasn't even halfway done. visualViewport.scale
     tells us a pinch is in progress; a small width-delta threshold ignores
     the spurious resize events some mobile browsers fire during a pinch even
     when the layout width has not actually changed. */
  let lastFitWidth = window.innerWidth;
  const refit = () => {
    const vv = window.visualViewport;
    if (vv && Math.abs(vv.scale - 1) > 0.02) return;
    const w = window.innerWidth;
    if (Math.abs(w - lastFitWidth) < 4) return;
    lastFitWidth = w;
    autoDetectScale(); applyScaleAndFont();
  };
  window.addEventListener('resize', debounce(refit, 200));
  window.addEventListener('orientationchange', () => setTimeout(() => { lastFitWidth = window.innerWidth; refit(); }, 250));
  if (window.visualViewport) window.visualViewport.addEventListener('resize', debounce(refit, 200));
