...
/* Briefly reveals the row's small corner number as a click confirmation,
   then fades it out. Only one timer at a time so rapid clicks don't leave
   a stale row's number stuck visible after the list re-renders. */
let leadNumFlashTimer = null;
function flashLeadNumber(id) {
  const row = document.querySelector('.lead-row[data-id="' + id + '"]');
  const num = row && row.querySelector('.lead-row-num');
  if (!num) return;
  if (leadNumFlashTimer) clearTimeout(leadNumFlashTimer);
  num.classList.add('show');
  leadNumFlashTimer = setTimeout(() => num.classList.remove('show'), 5000);
}
...
function contactIconsEmail(addr) {
  return `<span class="contact-icons"><button class="cicon-btn email" title="Email" data-act="email" data-addr="${escapeHtml(addr)}">${ICONS.envelope}</button></span>`;
}
...
function renderSelectedLead() {
  ...
  const phCompany = document.getElementById('phLeadCompany');
  const phOwner = document.getElementById('phLeadOwner');
  if (!l) {
    body.innerHTML = emptyPanelHtml('No lead selected', 'Choose a lead from the list to see their details here.');
    phCompany.textContent = 'Overview';
    phOwner.textContent = '';
    return;
  }
  phCompany.textContent = l.company;
  phOwner.textContent = l.name;
  ...
  body.innerHTML = `
  <div class="sl-scroll">
    <div class="kv-pairs sl-top-kv">
      <div class="kv-pair"><div class="kv-label">Monthly Revenue</div><div class="kv-value">${fmtMoney(l.avgDeposits)}</div></div>
      <div class="kv-pair"><div class="kv-label">Approval Amount</div><div class="kv-value pos">${fmtMoney(l.approval)}</div></div>
    </div>
    ...
    <div class="sl-section">
      ...
      <div class="field-grid">
        ...
        <div class="field-row-trio" style="grid-column:1/3">
          <div class="field-row"><span class="field-label">SSN</span><span class="field-val">${escapeHtml(ids.ssn)}</span></div>
          <div class="field-row"><span class="field-label">EIN</span><span class="field-val">${escapeHtml(ids.ein)}</span></div>
          <div class="field-row"><span class="field-label">DOB</span><span class="field-val">${escapeHtml(ids.dob)}</span></div>
        </div>
        <div class="field-row" style="grid-column:1/3">
          <span class="field-label">Company Address</span>
          <div class="field-address">
            <div>${escapeHtml(l.address.street)}</div>
            <div>${escapeHtml(l.address.city)}, ${escapeHtml(l.address.state)} ${escapeHtml(l.address.zip)} · ${escapeHtml(l.tz.split(' ')[0])}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="sl-section">
      ...
      <div class="contact-cols">
        <div class="contact-col">
          <div class="contact-col-label">Mobile</div>
          ${l.mobiles.map(m => `<div class="contact-row"><span class="contact-num">${m}</span>${contactIconsMobile(m, l.id)}</div>`).join('')}
        </div>
        <div class="contact-col">
          <div class="contact-col-label">Landline</div>
          ${l.landlines.map(n => `<div class="contact-row"><span class="contact-num">${n}</span>${contactIconsLandline(n)}</div>`).join('')}
        </div>
      </div>
      <div class="contact-col-label">Email</div>
      <div class="contact-emails">
        ${l.emails.map(e => `<div class="contact-row"><span class="contact-email">${escapeHtml(e)}</span>${contactIconsEmail(e)}</div>`).join('')}
      </div>
    </div>
      ...
      <div class="stmt-layout">
        <div class="stmt-list">
          ${l.statements.map(st => `<div class="stmt-row">
            ...
            <div class="stmt-figs">
              <div class="stmt-fig"><div class="sf-label">Deposits</div><div class="sf-val">${fmtMoney(st.deposits)}</div></div>
              <div class="stmt-fig"><div class="sf-label">Balance</div><div class="sf-val">${fmtMoney(st.endingBalance)}</div></div>
            </div>
          </div>`).join('')}
        </div>
        ${l.mtd ? `<div class="stmt-mtd-aside">
          <div class="stmt-fig"><div class="sf-label">MTD Deposits</div><div class="sf-val">${fmtMoney(l.mtd.deposits)}</div></div>
          <div class="stmt-fig"><div class="sf-label">MTD Balance</div><div class="sf-val">${fmtMoney(l.mtd.balance)}</div></div>
        </div>` : ''}
      </div>
      ${l.mtd ? `<div class="stmt-asof">${escapeHtml(l.mtd.month)} month-to-date · as of ${escapeHtml(l.mtd.asOf)}</div>` : ''}
    </div>
...
function copyLeadDetails(l) {
  const lines = [
    l.company, l.name + ' · ' + l.title, fmtAddress(l.address), '',
...
function openEditLead(l, focus) {
  openModal({
    ...
    body:
      ...
      '<div class="form-row"><label>Street address</label><input class="text-input" data-f="addrStreet" value="' + escapeHtml(l.address.street) + '"></div>',
    ...
    onMount(card, close) {
      ...
      card.querySelector('[data-save]').addEventListener('click', () => {
        card.querySelectorAll('[data-f]').forEach(inp => {
          ...
          if (k === 'addrStreet') { l.address.street = inp.value; return; }
...
/* ============================================================
   ...
   ============================================================ */
const COMMS_TABS = [
  { key: 'all' }, { key: 'messages' }, { key: 'calllog' }, { key: 'contacts' }, { key: 'email' }
];
...
/* The tab switcher lives in the panel header as static icon buttons (wired
   once in initLeadsPage) rather than a rebuilt row — this just reflects
   which one is active. */
function updateCommsTabIcons() {
  COMMS_TABS.forEach(t => {
    const btn = document.getElementById('commsTab' + t.key.charAt(0).toUpperCase() + t.key.slice(1));
    if (btn) btn.classList.toggle('on', leadsState.commsTab === t.key);
  });
}
...
function initLeadsPage() {
  ...
  document.getElementById('leadAppFileBtn').addEventListener('click', () => {
    const l = getLead(leadsState.currentLeadId);
    if (l) openDocViewer(l.id, 'application');
  });
  document.getElementById('leadHeaderCallBtn').addEventListener('click', () => {
    const l = getLead(leadsState.currentLeadId);
    if (l) startCall(l, l.mobiles[0]);
  });
  document.getElementById('leadHeaderSmsBtn').addEventListener('click', () => {
    if (getLead(leadsState.currentLeadId)) { leadsState.composeChannel = 'SMS'; switchCommsTab('messages'); }
  });
  document.getElementById('leadHeaderEmailBtn').addEventListener('click', () => {
    const l = getLead(leadsState.currentLeadId);
    if (l) { leadsState.composeTo = l.emails[0]; switchCommsTab('email'); }
  });
  COMMS_TABS.forEach(t => {
    document.getElementById('commsTab' + t.key.charAt(0).toUpperCase() + t.key.slice(1))
      .addEventListener('click', () => switchCommsTab(t.key));
  });
