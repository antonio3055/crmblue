...
function renderLeadList() {
  ...
  scroll.innerHTML = list.map(l => `
    <div class="lead-row ${l.id === leadsState.currentLeadId ? 'selected' : ''}" data-id="${l.id}">
      ...
      ${leadTrackingMark(l)}
      <div class="lead-avatar" style="background:${l.color}">${l.initials}</div>
...
function contactIconsLandline(num) {
  return `<div class="contact-icons"><button class="cicon-btn" title="Call" data-act="call" data-num="${num}">${ICONS.call}</button></div>`;
}
function contactIconsEmail(addr) {
  return `<div class="contact-icons"><button class="cicon-btn" title="Email" data-act="email" data-addr="${escapeHtml(addr)}">${ICONS.envelope}</button></div>`;
}
  ...
  <div class="sl-scroll">
    <div class="sl-identity">
      ...
      <div style="min-width:0">
        ...
        <div class="sl-revenue">${fmtMoney(l.revenue)}/yr</div>
        <div class="sl-status" style="color:${statusColor}">${escapeHtml(l.status)}</div>
      </div>
      <div class="sl-actions">
        <button class="sl-action-btn call" data-act="call" data-num="${l.mobiles[0]}">${ICONS.call}Call</button>
        <button class="sl-action-btn" data-act="sms" data-num="${l.mobiles[0]}">${ICONS.sms}SMS</button>
        <button class="sl-action-btn" data-act="email" data-addr="${escapeHtml(l.emails[0])}">${ICONS.envelope}Email</button>
      </div>
    </div>
